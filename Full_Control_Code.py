...
# Left / Right
    ...
    DRONE_EMERGENCY = [[0, 1, 1, 1, 0],[0, 1, 1, 1, 0]]


# Get the drone's NavData function
    def store_navdata():
        """
        Retruns the drone's NavData
        :return : NavData array
        """
...
# Creating a function that updates the LCDs
    def update_lcd():
        ...
        # Getting the velocity of the X,Y axis
        ui.lcdNumber_VX.display(navdata.vx)
        ui.lcdNumber_VY.display(navdata.vy)

        # Getting the drone's altitude
        ui.lcdNumber_VZ.display(navdata.altitude)
...
# Hand Gesture Control part
        if hands:
            ...
            if len(hands) == 2:
                ...
                if [fingers_L, fingers_R] == DRONE_TAKE_OFF:
                    _DroneTakeOff()
                    
                    
                if [fingers_L, fingers_R] == DRONE_UP:
                    _DroneUP()
                    

                if [fingers_L, fingers_R] == DRONE_DOWN:
                    _DroneDown()
                    
                            
                if [fingers_L, fingers_R] == DRONE_LEFT:
                    _DroneLeft()
                    

                if [fingers_L, fingers_R] == DRONE_RIGHT:
                    _DroneRight()
                    
                
                if [fingers_L, fingers_R] == DRONE_Clockwise:
                    _DroneClockwise()
                    

                if [fingers_L, fingers_R] == DRONE_CounterClockwise:
                    _DroneCounterClockwise()
                    

                if [fingers_L, fingers_R] == DRONE_Forward:
                    _DroneForward()
                    
                
                if [fingers_L, fingers_R] == DRONE_Backward:
                    _DroneBackward()
                    

                if [fingers_L, fingers_R] == DRONE_HOVER:
                    _DroneHover()
                    
                    
                if [fingers_L, fingers_R] == DRONE_LAND:
                    _DroneLand()
                    

                if [fingers_L, fingers_R] == DRONE_EMERGENCY:
                    _DroneEmergency()
    ...
    timer6.setInterval(30)
    timer6.timeout.connect(store_navdata)
    timer6.start()
